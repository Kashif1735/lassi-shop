...
// Disable Vercel's built-in body parser so we can handle multipart ourselves
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      let imageUrl = '';
      if (file) imageUrl = await uploadToCloudinary(file.buffer, file.filename);

      const { data, error } = await supabase
        .from('menu')
        .insert({ name, category, price: parseFloat(price), description: description || '', image: imageUrl })
        .select().single();
      if (error) return res.status(500).json({ error: error.message });
    ...
    if (req.method === 'DELETE') {
      ...
      const { data: existing } = await supabase
        .from('menu').select('image').eq('id', id).single();
